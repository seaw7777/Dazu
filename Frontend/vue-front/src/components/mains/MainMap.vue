  ...
  async created() {
    ...
    for (var i = 0; i < storeData.data.length; i++) {
      ...
      for (let index = 0; index < storeClass.data.length; index++) {
        ...
        element += '<br>';
        element += `<a href="http://https://i4d104.p.ssafy.io/class/detail/${id}" style="color:blue">${storeClass.data[index].class_name}&emsp;&emsp;&emsp;${storeClass.data[index].class_price}원</a>`;
      }
      element += '</div>';
